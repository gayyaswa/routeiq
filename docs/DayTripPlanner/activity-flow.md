...
Two classifiers are available, each using a different evidence source:
...
Both implement the same `ActivityClassifier` interface. The rest of the pipeline
...
Three ranker strategies, automatically selected based on user context and rating availability:
...
| `LLMSyntheticRatingProvider` | LLM-generated descriptions | Offline / no API access; used as eval baseline (default) |
...
## Step 6 — Context: semantic + knowledge-graph enrichment

After enrichment, `query_poi_context` indexes the rated POI Wikipedia descriptions into
ChromaDB and retrieves semantically matched, KG-enriched context for each stop.

```
query_poi_context(
    preferences     = ["hiking", "kids"],
    rated_pois_json = "<rate_pois JSON output>"
)
```

For each of the 5 selected POIs it returns:
- **Semantic match score** — cosine similarity of the POI's Wikipedia chunks against the user's preferences
- **Wikipedia evidence** — the most relevant passage from the POI's indexed description
- **City + region** — from the KG `LOCATED_IN` edges
- **Nearby POIs** — from the KG `NEAR_POI` edges ("2 min from Crissy Field")

This context block is passed directly into the narrate prompt. It grounds `why_visit` sentences
in real KG relationships and Wikipedia evidence rather than LLM invention.

No external API call — ChromaDB is local, KG is in-memory.

---

## Step 7 — Narrate: different voice for each track
...
## Step 8 — Graceful fallback (when an activity isn't available)
...
```
              ...
              └─────────┬──────────┘
                        │
              ┌─────────▼──────────┐
              │  query_poi_context  │  ChromaDB: index Wikipedia descriptions
              │                     │  KG: city, region, NEAR_POI relationships
              │                     │  → semantic match + evidence per preference
              └─────────┬──────────┘
                        │
              ┌─────────▼──────────┐
              │   Narrate Node      │  Track 1: activity evidence + quality signals
              │                     │  Track 2: scenic quality + rating
              │                     │  all claims grounded in provider + KG data
              └─────────┬──────────┘
                        │
                   Day trip plan
...
## LLM Synthetic enrichment — default and fallback

`LLMSyntheticRatingProvider` is the default (`RATING_PROVIDER=llm_synthetic`) and serves
multiple roles alongside the optional real-data providers:

- **Zero credentials needed** — works with no API keys; the recommended starting point
- **Eval baseline** — establishes the quality floor before real provider data is introduced
- **POI coverage fallback** — if TripAdvisor or Tavily returns no results for an obscure POI,
  LLM synthetic fills the gap rather than leaving the stop card empty

In the eval, comparing `llm_synthetic` vs `tripadvisor` or `tavily_enrichment` on the same
golden dataset directly measures the quality lift real provider data provides.
